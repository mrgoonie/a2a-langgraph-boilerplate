...
def update_crew(crew_id: UUID, crew: crew_schema.CrewCreate, db: Session = Depends(get_db)):
    db_crew = crew_service.get_crew(db, crew_id=crew_id)
    if db_crew is None:
        raise HTTPException(status_code=404, detail="Crew not found")
...
def delete_crew(crew_id: UUID, db: Session = Depends(get_db)):
    db_crew = crew_service.get_crew(db, crew_id=crew_id)
    if db_crew is None:
        raise HTTPException(status_code=404, detail="Crew not found")
...
def execute_prompt(crew_id: UUID, prompt: prompt_schema.PromptCreate, db: Session = Depends(get_db)):
    db_crew = crew_service.get_crew(db, crew_id=crew_id)
    if db_crew is None:
        raise HTTPException(status_code=404, detail="Crew not found")
