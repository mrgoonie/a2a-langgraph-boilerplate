...
def update_agent(agent_id: UUID, agent: agent_schema.AgentCreate, db: Session = Depends(get_db)):
...
def add_tool_to_agent(agent_id: UUID, tool_id: UUID, db: Session = Depends(get_db)):
