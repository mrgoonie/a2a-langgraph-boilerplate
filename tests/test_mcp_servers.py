...
def test_create_mcp_server(client: TestClient, db_session: Session):
...
def test_read_mcp_servers(client: TestClient, db_session: Session):
    client.post(
        "/mcp_servers/",
...
def test_read_mcp_server(client: TestClient, db_session: Session):
    response = client.post(
        "/mcp_servers/",
        json={"name": "Test MCP Server", "url": "http://localhost:8001"},
    )
    mcp_server_id = response.json()["id"]
    response = client.get(f"/mcp_servers/{mcp_server_id}")
    assert response.status_code == 200
    data = response.json()
    assert data["name"] == "Test MCP Server"
    assert data["id"] == mcp_server_id

def test_update_mcp_server(client: TestClient, db_session: Session):
    response = client.post(
        "/mcp_servers/",
        json={"name": "Test MCP Server", "url": "http://localhost:8001"},
    )
...
def test_delete_mcp_server(client: TestClient, db_session: Session):
    response = client.post(
        "/mcp_servers/",
        json={"name": "Test MCP Server", "url": "http://localhost:8001"},
    )
    mcp_server_id = response.json()["id"]
    response = client.delete(f"/mcp_servers/{mcp_server_id}")
    assert response.status_code == 200
    data = response.json()
    assert data["name"] == "Test MCP Server"
    assert data["id"] == mcp_server_id
...
def test_get_mcp_server_tools(mock_streamablehttp_client, client: TestClient, db_session: Session):
    ...
    with patch("app.services.mcp_server.ClientSession") as mock_clientsession:
        ...
        response = client.get(f"/mcp_servers/{mcp_server_id}/tools")
        assert response.status_code == 200
        data = response.json()
