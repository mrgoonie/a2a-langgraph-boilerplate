...
def simple_crew(db_session):
    ...
    supervisor = next((agent for agent in crew.agents if agent.role == 'supervisor'), None)
    assert supervisor is not None, "Supervisor not found in the created crew"
    supervisor.system_prompt = """You are a helpful assistant. If the user says hello, you say hello back and then FINISH. For any other request, you must state that you cannot help and then FINISH."""
    db_session.commit()
...
def travel_crew(db_session):
    tavily_server = db_session.query(mcp_server_model.McpServer).filter_by(name="Tavily").first()
    if not tavily_server:
        ...
        tavily_server = mcp_server_service.create_mcp_server(db=db_session, mcp_server=tavily_server_data)
...
def test_simple_direct_response(db_session, simple_crew):
    ...
    agent_messages = [m for m in messages if get_sender(m) not in ['user', 'supervisor', None]]
    assert len(agent_messages) == 0, f"Unexpected agent messages found: {[get_sender(m) for m in agent_messages]}"
